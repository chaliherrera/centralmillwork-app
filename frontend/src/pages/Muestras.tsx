  ...
  return (
    ...
    >
      ...
      <div className="text-[10px] text-gray-500 space-y-0.5">
        ...
        {m.version_actual > 1 && (
          <div className="text-purple-700 font-medium">V{m.version_actual}</div>
        )}
      </div>
    </button>
  )
...
function CrearMuestraModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const mut = useMutation({
    mutationFn: (body: CreateMuestraInput) => muestrasService.create(body),
...
function DetalleMuestraDrawer({ id, onClose, onChange }: { id: number; onClose: () => void; onChange: () => void }) {
  ...
  const [tab, setTab] = useState<'overview' | 'ocs' | 'envios' | 'timeline'>('overview')
              ...
              {tab === 'overview' && (
                <div className="space-y-4">
                  ...
                  {data.versiones.length > 0 && (
                    <div>
                      <div className="text-xs font-semibold text-gray-500 uppercase mb-2">Versiones</div>
                      <div className="space-y-2">
                        {data.versiones.map((v) => (
                          <div key={v.id} className="bg-gray-50 rounded-lg p-3 text-sm">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-semibold text-purple-700">V{v.version_numero}</span>
                              {v.op_numero && <span className="text-xs text-gray-500">OP: {v.op_numero} · {v.op_status}</span>}
                            </div>
                            {v.especificaciones && <p className="text-xs text-gray-700 whitespace-pre-wrap">{v.especificaciones}</p>}
                            {v.razon_de_revision && <p className="text-xs text-red-700 mt-1"><b>Razón revisión:</b> {v.razon_de_revision}</p>}
                          </div>
                        ))}
                      </div>
                    </div>
