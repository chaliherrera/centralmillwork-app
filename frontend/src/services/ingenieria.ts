...
export interface IngDealEnCurso { proyecto_id: number; codigo: string; nombre: string; cliente: string | null; estado: string; deal_estado: DealEstado; fecha_objetivo: string | null; fecha_realista: string | null; n_tareas: number; portal_token: string | null; deal_aprobado_at: string | null; cliente_rechazo: boolean }
...
export const ingenieriaService = {
  ...
  confirmarReserva: (proyectoId: number, asignaciones?: { id: number; asignado_nombre: string }[]) =>
    api.post<ApiResponse<{ aceptadas: number }>>(`/ingenieria/reserva/${proyectoId}/confirmar`, { asignaciones }).then((r) => r.data),
